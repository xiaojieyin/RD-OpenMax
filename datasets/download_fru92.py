...
DATASET_DIR = os.path.join(DATA_DIR, 'Fru')
DATASET_NAME = 'Fru'
...
if __name__ == '__main__':
    ...
    print("Downloading Fru dataset files...")
    download('fru92_images', 'fru92.zip')
    download('fru92_lists', 'fru92_lists.zip')

    mkdir(os.path.join(DATASET_DIR, 'train'))
    mkdir(os.path.join(DATASET_DIR, 'val'))

    print("Parsing train split...")
    with open(os.path.join(DATASET_DIR, 'fru92_lists/fru_train.txt'), 'r') as f:
        ...
        for i in tqdm(range(len(files))):
            ...
            os.system('cp {} {}'.format(os.path.join(DATASET_DIR, 'fru92_images', repr(dir_name)),
                                        os.path.join(DATASET_DIR, 'train', repr(category), file_name)))

    print("Parsing test split...")
    with open(os.path.join(DATASET_DIR, 'fru92_lists/fru_val.txt'), 'r') as f:
        files = [l.strip() for l in f.readlines()]
        for i in tqdm(range(len(files))):
            line = files[i]
            dir_name, _ = line.split(' ')
    ...
    print("Finished building Fru92 dataset")
