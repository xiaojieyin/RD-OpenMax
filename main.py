...
from src.confusion_pytorch import PairwiseConfusion, EntropicConfusion
import torch
...
def main():
    ...
    if args.representation == 'GAvP':
        ...
        if args.input_size >= 128:
            ...
            input_dim = 512 if args.arch.startswith('mpncovresnet50') else 128
            dimension_reduction = None if args.pretrained else 128
    ...
    # summary(model.cuda(), input_size=(3, args.input_size, args.input_size), batch_size=-1)
    # plot network
    ...
    if evaluate_transforms is not None:
        evaluate_loader = torch.utils.data.DataLoader(
            OpenSetImageFolder(valdir, evaluate_transforms, seed=args.seed, num_classes=args.num_classes),
            batch_size=args.batch_size, shuffle=False,
            num_workers=args.workers, pin_memory=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # writer.add_scalars('train_classifier/loss', {'train_loss': trainObj, 'test_loss': valObj}, epoch)
        # writer.add_scalars('train_classifier/accuracy', {'train_acc': top1, 'test_acc': prec1}, epoch)
        # update stats
    ...
    if evaluate_transforms is not None:
        ...
        loss, top1, top5 = validate(evaluate_loader, model, criterion)
        # writer.add_text('train_classifier/evaluate', 'Prec@1 {:.3f} Prec@5 {:.3f}'.format(top1, top5))
    # writer.close()
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # confusion loss
        if args.loss == 'p':
            loss += 10 * PairwiseConfusion(input)
        elif args.loss == 'e':
            loss += 10 * EntropicConfusion(input)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    torch.save(state, filename[0], _use_new_zipfile_serialization=False)
    if is_best:
