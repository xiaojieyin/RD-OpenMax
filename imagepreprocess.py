...
def preprocess_strategy(dataset, input_size):
    evaluate_transforms = None
    if dataset in ['CUB', 'Dogs', 'Food', 'iNat', 'ImageNet_LT', 'CUB-200-Painting', 'CUB-200', 'NYUD2']:
        ...
        evaluate_transforms = transforms.Compose([
            transforms.Resize(input_size),
            CenterCropWithFlip(input_size),
            transforms.Lambda(lambda crops: torch.stack([transforms.ToTensor()(crop) for crop in crops])),
            transforms.Lambda(lambda crops: torch.stack([normalize(crop) for crop in crops])),
        ])
    elif dataset.startswith('Aircraft') or dataset.endswith('Aircraft'):
        train_transforms = transforms.Compose([
